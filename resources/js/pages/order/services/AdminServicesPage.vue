<template>
                                    ...
                                    <td class="px-6 py-3.5">
                                        <div class="flex items-center gap-1">
                                            ...
                                            <span v-if="can.delete" class="relative group">
                                                <a href="#" @click.prevent="service.invoice_jobs_count ? null : confirmDelete(service)"
                                                    :class="service.invoice_jobs_count ? 'opacity-40 cursor-not-allowed' : ''"
                                                ...
                                                <span v-if="service.invoice_jobs_count" class="pointer-events-none absolute bottom-full right-0 mb-1 whitespace-nowrap rounded bg-gray-800 px-2 py-1 text-xs text-white opacity-0 group-hover:opacity-100 transition-opacity">In use — cannot delete</span>
                                            </span>
                                        </div>
                                    </td>
                                </tr>
    ...
    <DeleteModal :show="!!deleteTarget" @confirm="deleteService" @cancel="deleteTarget = null; deleteError = null"
        title="Delete Service" message="Are you sure you want to delete this service?" :error="deleteError" />
</template>
...
async function deleteService() {
    ...
    try {
        ...
        deleteError.value = e?.response?.data?.message ?? 'Failed to delete service.'
    }
}
