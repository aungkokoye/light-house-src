<template>
            ...
            <template v-else>
                <div class="mb-6 flex items-center gap-3">
                    ...
                    <div class="flex-1">
                        ...
                        <p class="text-sm text-gray-500 mt-0.5">Manage customer records.</p>
                    </div>
                    <RouterLink v-if="can.create" to="/order/customers/create"
                ...
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 mb-4">
                    <div class="relative">
                        ...
                        <input v-model="search" @input="debouncedFetch" type="text" placeholder="Search name, email, company or phone…"
                ...
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                    <div class="px-6 py-4 border-b border-gray-50 flex items-center justify-between">
                        <h2 class="font-semibold text-gray-900">All Customers</h2>
                        <div class="flex items-center gap-3">
                            <span class="text-xs text-gray-400">{{ meta.total ?? 0 }} total</span>
                            <select v-model="perPage" @change="fetchCustomers(1)"
                    ...
                    <div v-else-if="customers.length === 0" class="px-6 py-12 text-center text-sm text-gray-400">
                        No customers found.
                    </div>
                        ...
                        <table class="w-full text-sm">
                            <thead>
                                <tr class="text-left text-xs text-gray-400 border-b border-gray-50">
                                    ...
                                    <th class="px-6 py-3 font-medium cursor-pointer select-none hover:text-gray-600" @click="toggleSort('phone')">
                                        <span class="inline-flex items-center gap-1">
                                            Phone
                                            <span class="inline-flex flex-col leading-none">
                                                <svg class="w-2.5 h-2.5 -mb-0.5 transition-colors" :class="sortBy==='phone'&&sortDir==='asc'?'text-indigo-500':'text-gray-200'" viewBox="0 0 10 6" fill="currentColor"><path d="M5 0L10 6H0L5 0Z"/></svg>
                                                <svg class="w-2.5 h-2.5 transition-colors" :class="sortBy==='phone'&&sortDir==='desc'?'text-indigo-500':'text-gray-200'" viewBox="0 0 10 6" fill="currentColor"><path d="M5 6L0 0H10L5 6Z"/></svg>
                                            </span>
                                        </span>
                                    </th>
                            ...
                            <tbody class="divide-y divide-gray-50">
                                <tr v-for="c in customers" :key="c.id" class="hover:bg-gray-50/50 transition-colors">
                                    ...
                                    <td class="px-6 py-3.5 text-sm text-gray-600">{{ c.company_name ?? '—' }}</td>
                                    <td class="px-6 py-3.5 text-sm text-gray-600">{{ c.phone }}</td>
                                    <td class="px-6 py-3.5 text-xs text-gray-400">{{ formatDate(c.created_at) }}</td>
                                    <td class="px-6 py-3.5">
                                        <div class="flex items-center gap-1">
                                            <RouterLink v-if="can.view" :to="`/order/customers/${c.id}`"
                                            ...
                                            <RouterLink v-if="can.edit" :to="`/order/customers/${c.id}/edit`"
                                                class="p-1.5 text-indigo-600 hover:text-gray-600 hover:bg-gray-100 rounded-lg transition-colors">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                            ...
                                            <span v-if="can.delete" class="relative group">
                                                <a href="#" @click.prevent="c.invoices_count ? null : confirmDelete(c)"
                                                    :class="c.invoices_count ? 'opacity-40 cursor-not-allowed' : ''"
                                                    class="p-1.5 text-red-400 hover:text-gray-600 hover:bg-gray-100 rounded-lg transition-colors block">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
                                                    </svg>
                                                </a>
                                                <span v-if="c.invoices_count" class="pointer-events-none absolute bottom-full right-0 mb-1 whitespace-nowrap rounded bg-gray-800 px-2 py-1 text-xs text-white opacity-0 group-hover:opacity-100 transition-opacity">Has invoices — cannot delete</span>
                                            </span>
                                        </div>
                                    </td>
                                </tr>
                    ...
                    <div v-if="meta.last_page > 1" class="px-6 py-4 border-t border-gray-50 flex items-center justify-between">
                        ...
                        <div class="flex items-center gap-1">
                            <button @click="fetchCustomers(currentPage - 1)" :disabled="currentPage === 1"
                            ...
                            <template v-for="page in visiblePages" :key="page">
                                ...
                                <button v-else @click="fetchCustomers(page)"
                            ...
                            <button @click="fetchCustomers(currentPage + 1)" :disabled="currentPage === meta.last_page"
    ...
    <DeleteModal :show="!!deleteTarget" @confirm="deleteCustomer" @cancel="deleteTarget = null; deleteError = null"
        title="Delete Customer" message="Are you sure you want to delete this customer?" :error="deleteError" />
</template>
...
async function deleteCustomer() {
    ...
    try {
        await axios.delete(`/api/order/customers/${deleteTarget.value.id}`)
        deleteTarget.value = null
        fetchCustomers(currentPage.value)
    } catch (e) {
        deleteError.value = e?.response?.data?.message ?? 'Failed to delete customer.'
    }
}
...
async function fetchCustomers(page = 1) {
    ...
    try {
        ...
        const { data } = await axios.get('/api/order/customers', { params })
        ...
        sessionStorage.setItem('customer-list-back', '/order/customers?' + new URLSearchParams(finalParams).toString())
    } catch (e) {
...
onMounted(async () => {
    ...
    await fetchCustomers(Number(route.query.page) || 1)
    loading.value = false
})
</script>
