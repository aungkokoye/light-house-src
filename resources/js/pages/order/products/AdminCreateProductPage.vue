        ...
        <div class="max-w-xl mx-auto">
            ...
            <template v-else>
                <div class="mb-8 flex items-center gap-3">
                    <RouterLink to="/order/products" class="text-gray-400 hover:text-gray-600 transition-colors">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 19.5 8.25 12l7.5-7.5" />
                        </svg>
                    </RouterLink>
                    <div>
                        <h1 class="text-3xl font-bold text-gray-900">New Product</h1>
                ...
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                    <form @submit.prevent="submit" class="space-y-5">
                        <div>
                            ...
                            <input v-model="form.name" type="text" placeholder="e.g. Web Design Package"
                        ...
                        <div>
                            ...
                            <textarea v-model="form.description" rows="4" placeholder="Optional description…"
                        ...
                        <div>
                            <label class="block text-xs font-medium text-gray-600 mb-1.5">Per Price <span class="text-red-400">*</span></label>
                            <input v-model.number="form.per_price" type="number" min="0" placeholder="0"
                                class="w-full px-3 py-2 text-sm border rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-200 bg-gray-50"
                        ...
                        <div class="flex items-center justify-end gap-3 pt-2">
                            <RouterLink to="/order/products" class="px-4 py-2 text-sm text-gray-500 hover:text-gray-700 transition-colors">Cancel</RouterLink>
...
const form = ref({ name: '', description: '', per_price: 0 })
...
async function submit() {
    ...
    try {
        const { data } = await axios.post('/api/order/products', form.value)
        router.push(`/order/products/${data.id}`)
    } catch (e) {
