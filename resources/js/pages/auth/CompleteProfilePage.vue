        ...
        <div class="w-full max-w-lg">
            ...
            <div class="bg-white rounded-2xl shadow-xl shadow-gray-100 border border-gray-400 p-8">
                ...
                <form @submit.prevent="handleSubmit" class="space-y-4">
                    ...
                    <!-- reCAPTCHA -->
                    <div>
                        <div id="recaptcha-complete-profile"></div>
                        <p v-if="errors.recaptcha_token" class="text-xs text-red-500 mt-1">{{ errors.recaptcha_token[0] }}</p>
                    </div>
...
const isPending = route.query.pending === '1'
const { getToken, reset } = useRecaptcha('recaptcha-complete-profile')
...
async function handleSubmit() {
    ...
    generalError.value = ''
    try {
        await axios.post('/api/profile/company', { ...form, recaptcha_token: getToken() })
        localStorage.removeItem('token')
        router.replace(isPending ? '/login?pending=1' : '/')
    } catch (e) {
