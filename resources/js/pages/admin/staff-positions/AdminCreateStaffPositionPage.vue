    ...
    <div class="min-h-screen bg-gradient-to-br from-slate-50 via-white to-indigo-50 pt-24 pb-12 px-4">
        <div class="max-w-xl mx-auto">
            <div class="mb-8 flex items-center gap-3">
                ...
                <div>
                    <h1 class="text-3xl font-bold text-gray-900">New Staff Position</h1>
                    <p class="text-sm text-gray-500 mt-0.5">Add a new position to the system.</p>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                <form @submit.prevent="submit" class="space-y-5">
                    ...
                    <div class="flex items-center justify-end gap-3 pt-2">
                        <RouterLink to="/admin/staff-positions" class="px-4 py-2 text-sm text-gray-500 hover:text-gray-700 transition-colors">Cancel</RouterLink>
...
async function submit() {
    ...
    try {
        const { data } = await axios.post('/api/admin/staff-positions', form.value)
        router.push(`/admin/staff-positions/${data.id}`)
    } catch (e) {
