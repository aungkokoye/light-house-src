            ...
            <template v-else-if="role">
                <div class="mb-8 flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-gray-500 mt-0.5">Viewing <span class="font-medium text-gray-700">{{ role.name }}</span>.</p>
                        </div>
                    </div>
                    <RouterLink v-if="can.edit" :to="`/admin/roles/${role.id}/edit`"
                ...
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm divide-y divide-gray-50 mb-4">
                    <div class="px-6 py-4 flex items-center justify-between">
                        ...
                        <span class="text-sm font-mono text-gray-600">{{ role.id }}</span>
                    </div>
                    <div class="px-6 py-4 flex items-center justify-between">
                    ...
                    <div class="px-6 py-4 flex items-center justify-between">
                        ...
                        <span class="text-sm text-gray-600">{{ formatDate(role.created_at, true) }}</span>
                    </div>
                    <div class="px-6 py-4 flex items-center justify-between">
                        <span class="text-xs font-medium text-gray-400 uppercase tracking-wide">Updated At</span>
                        <span class="text-sm text-gray-600">{{ formatDate(role.updated_at, true) }}</span>
                    </div>
                    <div class="px-6 py-4 flex items-center justify-between">
                        <span class="text-xs font-medium text-gray-400 uppercase tracking-wide">Created By</span>
                        <div class="text-right">
                            <p class="text-sm text-gray-700 font-medium">{{ role.created_by_name }}</p>
                            <p class="text-xs text-gray-400">{{ role.created_by_email }}</p>
                        </div>
                    </div>
                </div>
