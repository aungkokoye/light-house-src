            ...
            <template v-else-if="service">
                ...
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm divide-y divide-gray-50">
                    <div class="px-6 py-4 flex items-center justify-between">
                        ...
                        <span class="text-sm font-mono text-gray-600">{{ service.id }}</span>
                    </div>
                    <div class="px-6 py-4 flex items-center justify-between">
                    ...
                    <div class="px-6 py-4">
                        ...
                        <p class="text-sm text-gray-600 whitespace-pre-wrap">{{ service.description ?? '—' }}</p>
                    </div>
                    <div class="px-6 py-4 flex items-center justify-between">
                        <span class="text-xs font-medium text-gray-400 uppercase tracking-wide">Created At</span>
                        <span class="text-sm text-gray-600">{{ formatDate(service.created_at, true) }}</span>
                    </div>
                    <div class="px-6 py-4 flex items-center justify-between">
                        <span class="text-xs font-medium text-gray-400 uppercase tracking-wide">Updated At</span>
                        <span class="text-sm text-gray-600">{{ formatDate(service.updated_at, true) }}</span>
                    </div>
                    <div class="px-6 py-4 flex items-center justify-between">
                        <span class="text-xs font-medium text-gray-400 uppercase tracking-wide">Created By</span>
                        <p class="text-sm text-gray-700 font-medium">{{ service.created_by?.name ?? '—' }}</p>
                    </div>
                </div>
            </template>
        </div>
