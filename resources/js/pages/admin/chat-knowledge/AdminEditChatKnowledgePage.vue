        ...
        <div class="max-w-2xl mx-auto">
            ...
            <template v-else>
                <div class="mb-8 flex items-center gap-3">
                    ...
                    <div>
                        <h1 class="text-3xl font-bold text-gray-900">Edit Knowledge Entry</h1>
                        <p class="text-sm text-gray-500 mt-0.5">#{{ route.params.id }}</p>
                    </div>
                </div>
...
async function submit() {
    ...
    try {
        await axios.put(`/api/admin/chat-knowledge/${route.params.id}`, form.value)
...
onMounted(async () => {
    ...
    if (!me) return
    try {
        const { data } = await axios.get(`/api/admin/chat-knowledge/${route.params.id}`)
        form.value = { category: data.category, title: data.title, content: data.content, active: data.active, sort_order: data.sort_order }
    } catch {
        router.push('/admin/chat-knowledge')
    } finally {
        loading.value = false
