<template>
            ...
            <template v-else>
                <div class="mb-6 flex items-center gap-3">
                    ...
                    <div class="flex-1">
                        ...
                        <p class="text-sm text-gray-500 mt-0.5">Manage all printing sites.</p>
                    </div>
                    <RouterLink v-if="can.create" to="/admin/sites/create"
                ...
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 mb-4">
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                        ...
                        <!-- Created from -->
                        <input v-model="filters.created_from" @change="fetchSites(1)"
                            type="date"
                        ...
                        <!-- Created to -->
                        <input v-model="filters.created_to" @change="fetchSites(1)"
                            type="date"
                            class="text-sm border border-gray-300 rounded-lg px-3 py-2 bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-200 text-gray-600" />
                    </div>
                ...
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                    <div class="px-6 py-4 border-b border-gray-50 flex items-center justify-between">
                        <h2 class="font-semibold text-gray-900">All Sites</h2>
                        <div class="flex items-center gap-3">
                            <span class="text-xs text-gray-400">{{ meta.total ?? 0 }} total</span>
                            <select v-model="perPage" @change="fetchSites(1)"
                    ...
                    <div v-if="sitesLoading" class="flex items-center justify-center py-12">
                    ...
                    <div v-else-if="sites.length === 0" class="px-6 py-12 text-center text-sm text-gray-400">
                        No sites found.
                    </div>
                    ...
                    <div v-else class="overflow-x-auto">
                        <table class="w-full text-sm">
                            ...
                            <tbody class="divide-y divide-gray-50">
                                <tr v-for="site in sites" :key="site.id" class="hover:bg-gray-50/50 transition-colors">
                                    <td class="px-6 py-3.5 text-xs font-mono text-gray-400">{{ site.id }}</td>
                                    <td class="px-6 py-3.5 font-medium text-gray-900">{{ site.name }}</td>
                                    <td class="px-6 py-3.5 text-gray-500 max-w-xs truncate">{{ site.address ?? '—' }}</td>
                                    <td class="px-6 py-3.5 text-gray-500">{{ site.phone ?? '—' }}</td>
                                    <td class="px-6 py-3.5 text-xs text-gray-400">{{ formatDate(site.created_at) }}</td>
                                    <td class="px-6 py-3.5">
                                        <div class="flex items-center gap-1">
                                            <RouterLink v-if="can.view" :to="`/admin/sites/${site.id}`"
                                            ...
                                            <a v-if="can.delete" href="#" @click.prevent="confirmDelete(site)"
                    ...
                    <div v-if="meta.last_page > 1" class="px-6 py-4 border-t border-gray-50 flex items-center justify-between">
                        ...
                        <div class="flex items-center gap-1">
                            <button @click="fetchSites(currentPage - 1)" :disabled="currentPage === 1"
                            ...
                            <template v-for="page in visiblePages" :key="page">
                                ...
                                <button v-else @click="fetchSites(page)"
                            ...
                            <button @click="fetchSites(currentPage + 1)" :disabled="currentPage === meta.last_page"
                                class="px-3 py-1.5 text-xs rounded-lg border border-gray-100 text-gray-500 hover:bg-gray-50 disabled:opacity-30 disabled:cursor-not-allowed transition-colors">
                                Next
                            </button>
                        </div>
    ...
    <DeleteModal :show="!!deleteTarget" @confirm="deleteSite" @cancel="deleteTarget = null"
...
function debouncedFetch() {
    ...
    searchTimer = setTimeout(() => fetchSites(1), 350)
}
...
async function deleteSite() {
    deleting.value = true
    try {
        await axios.delete(`/api/admin/sites/${deleteTarget.value.id}`)
...
async function fetchSites(page = 1) {
    ...
    try {
        ...
        const { data } = await axios.get('/api/admin/sites', { params })
        sites.value = data.data
        meta.value  = { total: data.total, from: data.from, to: data.to, last_page: data.last_page }
        can.value   = data.can ?? {}
        currentPage.value = data.current_page
    } catch (e) {
        console.error('fetchSites error', e?.response?.status)
    } finally {
